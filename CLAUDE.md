...
The decision is memoised per `(notifiable, notification)` and read twice: by `via()`, and by `InboxChannel` asking whether `live` survived. The notifiable is a **`WeakMap` key, not an `spl_object_id`** — an object id is reused once its object is collected, and a worker sending thousands of notifications would eventually answer one recipient's question with another's decision. The notification cannot be a key at all: `NotificationSender` clones it once for the send and again for **every channel**, so the two call sites never hold the same object and an identity-keyed memo would silently never hit. `TypedNotification::deliveryToken()` is what identifies it across those clones — a property, because `clone` copies it.

**`Channels/InboxChannel` writes the row and fires the one broadcast**, and resolves both of its collaborators per send rather than taking them in the constructor. Laravel's `ChannelManager` is a singleton that caches the channel object it built for the first delivery, so anything injected here outlives every scope reset — pinning the scoped `DeliveryResolver`, and the preference cache behind it, for the lifetime of an Octane worker. `Channels/LiveChannel` delivers nothing on purpose — `live` only decides whether `announce: true` sits on the payload `InboxChannel` broadcasts. The class exists so `live` has a `laravelChannel()` like every other channel and needs no special case in the chain.

**`Support/PreferenceResolver` is gate 3**, three tiers deep (type row → `group:` row → the type's default), serving every lookup for one recipient from a single query. `null` means undecided, **not off** — that is what lets a new type ship with a sensible default and no backfill. Resolution is **row-wise**: the row that answers `enabled` also answers the nullable `bypass_suppression`, read back together as an `Enums/ChannelPreference` (`Off` / `On` / `WhenAway` / `Always`). `Always` — stored, or declared by the type's `always:` list — skips gate 4; a type row never inherits its group row's bypass.
...
**A `locked` channel is always known**, whether or not an explicit `available` list repeats it — `NotificationDefinition::channels()` prepends the locked ones. A list that could omit one would drop the inbox row, which is the single delivery `locked` exists to guarantee, while `isLocked()` went on answering true for it.
...
- **A renamed morph key column has to be added to `getFillable()`, not waved through by `isFillable()`.** `fill()` runs `fillableFromArray()` first, which intersects the incoming attributes with `getFillable()`; a renamed key is dropped there and `isFillable()` is never consulted for it. An override on that method would only ever answer for the default column, which already sits in `$fillable` literally — and every write would land with a null recipient.
